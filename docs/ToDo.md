...
CHECK TIMELOG DESCRIPTION IN APP / AVATAR BUTTON
...
Subsections are left-aligned, bold, 12 pt, same blue as sections. Sub-subsections are the same except except 11 pt.
...
Table rows must follow the following pattern: first row white, second row (#F2F7FB) then repeat.
```
